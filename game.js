...
// ---------- Drabina oparta o tylną ścianę domu (plansza-6) ----------
const LADDER_POS = { x: -164.9, z: -9.5 };
const LADDER_TOP_Y = 8.6;
(function addHouseLadder() {
  const g = new THREE.Group();
  const woodMat = toonMat(0x8B5A2B);
  const lean = 0.18;
  [-0.45, 0.45].forEach((off) => {
    const rail = new THREE.Mesh(new THREE.BoxGeometry(0.12, LADDER_TOP_Y + 0.6, 0.12), woodMat);
    rail.position.set(0, (LADDER_TOP_Y + 0.6) / 2, off);
    rail.rotation.z = lean;
    rail.castShadow = true;
    g.add(rail);
  });
  const rungs = 11;
  for (let i = 1; i <= rungs; i++) {
    const y = (LADDER_TOP_Y / (rungs + 1)) * i;
    const rung = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.08, 1.0), woodMat);
    rung.position.set(-y * Math.tan(lean), y, 0);
    rung.castShadow = true;
    g.add(rung);
  }
  g.position.set(LADDER_POS.x, 0, LADDER_POS.z);
  scene.add(g);
})();
...
(function addWillowByGarage2() {
  ...
  willow.position.set(-135.4, 0, -7.2);
  scene.add(willow);
  FARM_BLOCKERS.push({ x1: -136.2, x2: -134.6, z1: -8.0, z2: -6.4 });
})();
...
function beginMartenEscape() {
  ...
  martenEscapeWaypoints = [
    ...
    { x: PLOT6_GROUND_X_MAX - 4, z: PLAY_Z_MAX - 6 },  // wjazd na planszę z domem
    { x: LADDER_POS.x - 6, z: PLAY_Z_MAX - 8 },        // OBIEGA DOM Z TYŁU
    { x: LADDER_POS.x - 5, z: LADDER_POS.z },          // zza domu w stronę drabiny
    { x: LADDER_POS.x - 1.1, z: LADDER_POS.z }         // pod samą drabinę
  ];
  martenEscapeIndex = 0;
}
...
function updateMarten(dt) {
  ...
  // --- Wspinaczka po drabinie na dach ---
  if (martenState === 'climbing') {
    martenMesh.position.x = LADDER_POS.x - 1.0;
    martenMesh.position.z = LADDER_POS.z;
    martenMesh.position.y += 3.2 * dt;
    martenMesh.rotation.y = Math.PI / 2;
    martenMesh.rotation.x = -0.9; // pionowa postawa na szczeblach
    if (martenMesh.position.y >= LADDER_TOP_Y) {
      martenMesh.position.y = LADDER_TOP_Y;
      martenMesh.rotation.x = 0;
      martenState = 'roof';
      martenWanderTimer = 0;
    }
    return;
  ...
  // --- Na dachu: chodzi w kółko, nie do złapania. Gdy Janusz podjedzie
  //     pod drabinę i ją widzi — krzyczy, a kuna spada. ---
  if (martenState === 'roof') {
    martenWanderTimer += dt;
    martenMesh.position.x = LADDER_POS.x - 1.0 + Math.sin(martenWanderTimer * 0.8) * 1.6;
    martenMesh.position.z = LADDER_POS.z + Math.cos(martenWanderTimer * 0.6) * 2.2;
    martenMesh.position.y = LADDER_TOP_Y;
    martenMesh.rotation.y = Math.sin(martenWanderTimer * 0.8) * 1.2;
    const dxl = tractor.position.x - LADDER_POS.x;
    const dzl = tractor.position.z - LADDER_POS.z;
    if (Math.hypot(dxl, dzl) < 7) startMartenShoutSequence();
    return;
  }

  // --- Spadanie z dachu (widoczne dla gracza) ---
  if (martenState === 'falling') {
    martenFallVy -= 16 * dt;
    martenMesh.position.y += martenFallVy * dt;
    martenMesh.rotation.z += 5 * dt;
    if (martenMesh.position.y <= 0.18) {
      martenMesh.position.y = 0.18;
      martenMesh.rotation.z = Math.PI / 2; // leży na boku, bez ruchu
      martenState = 'lying';
      setTimeout(finishMartenSequence, 4000); // 4 sekundy leżenia
    }
    return;
  }

  if (martenState === 'lying') return; // leży bez ruchu
...
const MARTEN_SHOUT_LINE = ['Pan Janusz: KURWA MAĆ!'];
const MARTEN_FELL_LINE = ['Pan Janusz: Kurna chata! Kuna tak się przestraszyła, że sama spadła z dachu!'];

function startMartenShoutSequence() {
  if (martenShoutDone) return;
  martenShoutDone = true;
  showDialogueModal(MARTEN_SHOUT_LINE, () => {
    martenFallVy = 0;
    martenState = 'falling';
  });
}

function finishMartenSequence() {
  showDialogueModal(MARTEN_FELL_LINE, () => {
    setTimeout(triggerVictory, 600);
  });
}

const MARTEN_INTRO_LINE = ['Pan Janusz: Ojej, kuna wlazła do hali! Jest strasznie sprytna — nie dasz rady jej złapać, trzeba jechać za nią i zobaczyć gdzie się schowa!'];
...
function makeLoopAudio(src, vol) {
...
const ambientTractor = makeLoopAudio('audio/Traktor.mp3', AMBIENT_VOL);
const ambientMeadow = makeLoopAudio('audio/Laka.mp3', AMBIENT_VOL);
const radioAudios = RADIO_STATIONS.map((st) => makeLoopAudio(st.file, RADIO_VOL));
...
function startAmbientIfNeeded() {
  ...
  audioStarted = true;
  ambientTractor.play().catch(() => {});
  ambientMeadow.play().catch(() => {});
}
...
function toggleRadioPower() {
  ...
  if (radioOn) {
    stopAllRadio();
    radioAudios[radioStation].play().catch(() => {});
  } else {
    stopAllRadio();
  }
  setAmbientVolume();
